...
std::array<float, 4> sample_sorted(const std::vector<Stop> &sorted, float t)
{
  ...
  {
    if (t > sorted[i].position) continue;
...
{
  if (stops.empty() || samples <= 0) return -1.f;
  ...
  for (int i = 0; i < samples; ++i)
  {
    float hue = 0.f;
    float sat = 0.f;
    hue_saturation(sample_sorted(sorted, sample_position(i, samples)),
                   hue,
                   sat);
