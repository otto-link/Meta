...
LinkedSliders::LinkedSliders(Attribute<glm::vec2> &attr,
                             ...
                             QWidget              *parent)
    : Control<glm::vec2>(ctx, parent), axes_theme_(theme()), state_(&attr.state())
{
  ...
  label->setStyleSheet(
      QString("color: %1; background: transparent;").arg(theme().ink_primary.name()));
  ...
  link_->setStyleSheet(
      QString("QToolButton { color: %1; background: %2; border: 1px solid %3; "
              "border-radius: 5px; } QToolButton:checked { border-color: %4; background: "
  ...
  for (int i = 0; i < 2; ++i)
  {
    ...
    if (ctx.default_value)
      axis_ctx.default_value =
          [get = ctx.default_value, key = attr.name(), i](const std::string &) -> std::any
      {
        const auto initial = get(key);
        if (auto pair = std::any_cast<glm::vec2>(&initial))
          return (*pair)[i];
        return {};
      };
    sliders_[i] = new ParamSlider(axes_[i], axis_ctx, this);
    layout->addWidget(sliders_[i]);
    connect(sliders_[i], &ControlBase::edit_started, this, [this] { begin_edit(); });
    connect(sliders_[i],
    ...
    connect(sliders_[i], &ControlBase::edit_ended, this, [this] { end_edit(); });
  }
  connect(link_,
          &QToolButton::toggled,
          this,
